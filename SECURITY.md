...
3. **Self-disarming.** Every agent carries `Bash`, and `talos` carries `Write` and `Edit` as well, so an agent can delete the entry from `.claude/settings.local.json` or overwrite the validator inside `vendor/`. This is a runtime layer under advisory documentation, not containment against an agent that wants out.
