...
- Convert the `## Assignment Compliance` block `@skills/assignment-compliance-check/SKILL.md` returns to JIRA Wiki Markup before passing it to `pr-summary` for the JIRA target, and keep the GitHub-Markdown original for the linked-GitHub-issue mirror. The block travels on every run with a linked ticket, its affirmative verdict included — a JIRA comment that stays silent when every criterion is met tells the ticket owner nothing (`@rules/code-review/general.md` *Two-Part CR Output* → *The tracker comment carries the same verdict — in all three cases*).
