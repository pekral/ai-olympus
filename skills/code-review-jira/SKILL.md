...
- The script accepts a bare key (`ACME-1234`), a `/browse/<KEY>` URL, or any URL containing `?selectedIssue=<KEY>`.
...
- **Repository ownership (hard gate).** For each linked PR, confirm it belongs to the current checkout by running `skills/_shared/assert-current-repo.sh <PR_URL>` before checking it out. A JIRA project can carry links to PRs across several repositories, so this is where a cross-repo review silently starts. Exit code `4` means the PR lives elsewhere: **skip that PR**, report the mismatch, and continue with the ones that match — do not check it out and do not post findings for it. Exit code `5` means ownership could not be proven (not a git checkout, or no github.com remote on any of them): stop and tell the caller to run from inside the target checkout.
...
The CR wrapper passes the returned block as an embedded block to `@skills/pr-summary/SKILL.md` **only when a block is returned** so each tracker (JIRA ticket, linked GitHub issue) receives **one consolidated comment** per CR run (per issue #498); on either skip status the wrapper embeds nothing and clean compliance is reported by the absence of the embedded block, consistent with every other "OK" section that is dropped rather than surfaced. **Do not embed** the block into the GitHub PR comment — the PR comment carries technical findings only.
  ...
  - run @skills/security-review/SKILL.md — **skipped when `SECURITY_OWNER=athena` is set by the caller.** When a **separate, concurrent** agent owns the security review, this inline pass repeats the same review over the same diff for no extra coverage. **Absence of the flag means it runs**, so coverage never drops by omission. **No agent this package ships sets the flag** (issue #179): `athena` is the single reviewer, so this inline pass *is* her security pass. The flag is for an external caller that genuinely runs a concurrent pass. When skipped, surface `security: owned by athena` on the summary line rather than implying a pass that did not happen here.
...
- Run conditionally:
  ...
  - Third-party API or service changes → ensure the **Third-Party API & Service Analysis** step from `@skills/code-review/SKILL.md` is executed for the diff, including the ordered documentation source walk (step 2) and, when it resolves no reference, the blocking documentation request (step 7). Publish every request on the GitHub PR comment under `## Documentation Requests` (see Output Rules) **and** fold a plain-language one-liner per request into the JIRA *Clarifying questions* block below, so whichever tracker the answerer reads carries the ask.
...
- **Consolidation contract (issue #498):** invoke `pr-summary` exactly once for the JIRA ticket, passing the conditional blocks together (clarifying questions first, then assignment compliance) so the reader gets **one** comment per CR run. When `@skills/assignment-compliance-check/SKILL.md` returned a markdown block (i.e. at least one Critical gap was detected), first convert that block to JIRA Wiki Markup per `@rules/jira/general.md` (`## ` → `h2. `, `**bold**` → `*bold*`, `` `code` `` → `{{code}}`, `- ` → `* `, Markdown link `[label]` + `(url)` → `[label|url]`) and pass it as an embedded block — `pr-summary` appends it verbatim after `How to test`.
When the compliance check returned the `no critical gaps — assignment compliance block omitted` status, **do not** pass that block; clean compliance is reported by its absence (no "Verdict: clean" line, no "What is satisfied" list). `pr-summary` then POSTs a new comment via `skills/code-review-jira/scripts/upsert-comment.sh` (JIRA MCP server fallback on exit code 2/3). Every CR run creates a **fresh JIRA comment** — the always-new comment convention means the chronological sequence of JIRA comments is the audit trail across runs, consistent with GitHub. Never post a separate JIRA comment for clarifying questions or assignment compliance on top of it.
...
- **Verify the published JIRA body contains no leaked Markdown.** Before / when publishing, confirm the body uses only Wiki Markup — no `**` / `__`, no `#`/`##` ATX headings, no `` ` ``/```` ``` ````, no `- ` bullets, no Markdown `[label]` + `(url)` links. Any such artifact must be converted per `@rules/jira/general.md` so the JIRA UI never shows raw markup characters.
...
- **Omit empty sections entirely.** The header block (Status / Counts / Last updated / Linked-tracker mirror), `## Functional Review`, and the final `Summary` line are always rendered in the GitHub PR comment. The `Coverage:` header line, the `## Coverage` section, and the `coverage …` slot in the summary line are all conditional — render them **only** when the coverage gate produced something to report (uncovered changed lines or unavailable / non-runnable tooling, both Critical findings). When every changed line is at 100% coverage and the tool ran successfully, drop all three coverage surfaces; the Counts line is the clean signal.
Every other section — `Findings` (including each severity sub-heading), `Documentation Requests`, `Refactoring (DRY / tech debt)`, `Refactoring proposals`, and `Database Analysis` — appears **only when it has at least one item**. Never emit `None.` / `Not applicable.` / `n/a` / `100%` placeholders for empty sections or omitted coverage surfaces; drop the whole heading and body instead. **History across CR runs** is preserved by the chronological sequence of always-new GitHub PR comments — never re-create a `Previous CR Status` section in the body.
- **Two-part output (`## Technical Review` / `## Functional Review`).** Structure the GitHub PR comment into `## Technical Review` (wraps every section below, unchanged) followed by `## Functional Review` (always rendered — the one exception to "omit empty sections" above) per `@rules/code-review/general.md` *Two-Part CR Output — Technical & Functional Review* — the canonical contract lives there, not here. The JIRA non-technical comment (produced by `pr-summary`) never includes either heading.
...
- **`## Documentation Requests` section (issue #151).** Render this section on the GitHub PR comment — after `## Findings`, before `## Refactoring (DRY / tech debt)` — **only** when the Third-Party API & Service Analysis step 7 in `@skills/code-review/SKILL.md` produced at least one blocking documentation request. Each entry carries the vendor / service, the version in use (or an explicit `could not determine`), the endpoints / SDK methods / webhook events being verified, and the one-sentence ask for the documentation link. The JIRA comment carries the same asks in plain language through the *Clarifying questions* block, never this technical section. Omit the heading entirely when no request exists.
- **Late-iteration report scope.** When the caller passed `iteration > 2`, the posted GitHub PR comment carries Critical and Moderate findings only, plus the `Report scope:` header line next to the unchanged real `Counts:` numbers (`@rules/code-review/general.md` *Late-Iteration Report Scope — Critical & Moderate Only (CR iteration > 2)*). The narrowing is a rendering filter — every review step still runs in full, and the convergence gate stays `Critical + Moderate == 0`. The JIRA non-technical comment (produced by `pr-summary`) is unaffected.
- **`## Excluded per assignment` section (issue #17).** This wrapper's step 1 (`load-issue.sh`) already loads the JIRA issue description/comments and the linked GitHub PR's description and review comments — exactly the first-class assignment sources the Assignment-Declared Test-Only Conditions — Exclusion Gate (`@rules/code-review/general.md` *Assignment-Declared Test-Only Conditions — Exclusion Gate (issue #17)*) requires for detection condition 1. Render the section on the GitHub PR comment only when `@skills/code-review/SKILL.md` moved at least one finding into it; omit it entirely otherwise. The JIRA non-technical comment (produced by `pr-summary`) never includes this section.
...
- When the diff touches database operations (per the trigger list in `@skills/code-review/SKILL.md` Specialized Reviews), the posted GitHub PR comment must include a dedicated `## Database Analysis` section **before** `## Coverage`. The section reports only the `mysql-problem-solver` findings (with severity mirroring Critical / Moderate / Minor), a one-sentence fix category, and a concrete SQL / query-builder rewrite, index DDL, or batch-operation snippet implementing that fix per `@rules/sql/optimalize.md` (issue #132) — never a category label alone. Do not include the queries / migrations inspected list or any EXPLAIN / static-analysis summary — those stay inside the internal investigation.
When no DB operations are present, omit the section entirely. The JIRA non-technical comment (produced by `pr-summary`) never includes this section.
...
- The JIRA Wiki Markup conversion (`h2.` / `h3.` headings, `*bold*`, `_italic_`, `{{inline}}`, `{code:php} ... {code}`, `*` / `#` bullets, `[label|url]`, `{quote}`) is handled by `@skills/pr-summary/templates/pr-summary-jira.md` per `@rules/jira/general.md`. Do not "translate" the output back to GitHub Markdown when posting via `acli` / JIRA MCP server, and never let a raw Markdown control character (`**`, `#`, `` ` ``, `- `) reach the published comment — the JIRA UI would show it literally.
