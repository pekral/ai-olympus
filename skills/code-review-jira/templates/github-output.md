...
> Render only when at least one in-scope refactoring item exists. Only items on lines touched by this PR. Each item must reduce tech debt — no stylistic preferences. Omit the entire section when there are no items, and omit it entirely when the report scope is narrowed (`iteration > 2`).
