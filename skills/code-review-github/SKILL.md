...
### 1. Load Context
- **Repository ownership (hard gate, runs first).** Before loading anything, confirm the PR belongs to the current checkout by running `skills/_shared/assert-current-repo.sh <URL>`. Exit code `4` means the PR lives in a different repository: **stop**, report the mismatch, and never review it from here — the review would read a foreign diff and post findings to another project's backlog. Exit code `5` means ownership could not be proven (not a git checkout, or no github.com remote on any of them): stop and tell the caller to run from inside the target checkout. Only a zero exit permits the flow to continue — every non-zero exit is a hard stop, and the deterministic loader's "exit 2/3 → fall back to the MCP server" convention never applies to this guard: there is no fallback for an ownership verdict.
...
- If no findings:
    - post the header block (Status / Counts / Last updated / Issue tracker summary), `## Technical Review` (empty body), `## Functional Review` ("All stated assignment requirements are satisfied."), and the final `Summary` line. The `Coverage:` header line, the `## Coverage` section, and the `coverage …` slot in the summary line are all dropped when every changed line is at 100% coverage and the tool ran successfully — only render them when the coverage gate produced uncovered changed lines (Critical findings), unavailable / non-runnable coverage tooling (Critical finding), or — the sanctioned savings-mode exception, `@rules/code-review/general.mdc` *Validation & Coverage Gate* — a `deferred to apollon` verdict from an isolated-worktree CR pass (non-Critical: render `Coverage: deferred to apollon (isolated worktree, no vendor/)`). Omit every other omit-if-empty subsection inside `## Technical Review`. Do not append a "No findings identified" line — the Counts line `Critical 0 · Moderate 0 · Minor 0 · Refactoring 0` already signals the clean state and the omitted sections confirm there is nothing to fix.
...
- No praise
- No “what was checked”
- **Omit empty sections entirely.** The header block (Status / Counts / Last updated / Issue tracker summary), `## Functional Review`, and the final `Summary` line are always rendered in the PR comment. The `Coverage:` header line, the `## Coverage` section, and the `coverage …` slot in the summary line are all conditional — render them **only** when the coverage gate produced something to report (uncovered changed lines or unavailable / non-runnable tooling, both Critical findings; or a savings-mode `deferred to apollon` verdict, non-Critical — the sanctioned exception in `@rules/code-review/general.mdc` *Validation & Coverage Gate*). When every changed line is at 100% coverage and the tool ran successfully, drop all three coverage surfaces; the Counts line is the clean signal. Every other section — `Findings` (including each severity sub-heading), `Refactoring (DRY / Tech Debt Reduction)`, `Refactoring Proposals`, and `Database Analysis` — appears **only when it has at least one item**. Never emit `None.` / `Not applicable.` / `n/a` / `100%` placeholders for empty sections or omitted coverage surfaces; drop the whole heading and body instead. **History across CR runs** is preserved by the chronological sequence of always-new PR comments — never re-create a `Previous CR Status` section in the body.
...
- The posted PR comment includes a `## Coverage` section before the summary line **only** when the coverage gate has something to report — uncovered changed lines (Critical findings), unavailable / non-runnable coverage tooling (Critical finding), or a savings-mode `deferred to apollon` verdict (non-Critical — the sanctioned isolated-worktree exception in `@rules/code-review/general.mdc` *Validation & Coverage Gate*; render `Coverage: deferred to apollon (isolated worktree, no vendor/)`, with the executed verdict landing in `apollon`'s own handoff instead). When every changed line is at 100% coverage and the tool ran successfully, omit the `## Coverage` section, the `Coverage:` header line, and the `coverage …` slot from the summary line per `@skills/code-review/SKILL.md` Output Rules. The coverage gate itself (per the Coverage gate in `@skills/code-review/SKILL.md`) still runs on every review; only the user-visible section is short-circuited.
