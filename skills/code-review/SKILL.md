...
7. **Request the documentation link when no source resolves.** When step 2 yields nothing for an API or service — no URL cited in the issue / PR, nothing vendored in the repository, and the online lookup either cannot be matched to the version in use or cannot be performed at all (no lookup tool available in this run) — raise the **Moderate** finding from step 6's fallback *and* publish a concrete, answerable **blocking documentation request** alongside it. A bare Moderate finding is **not** a sufficient output: the author must be able to close it with a single link. Each request carries:
