...
2. **Locate the official public reference for each one, walking these sources in order and stopping at the first that resolves.** The order is binding — a lower source never overrides a higher one that resolved:
   1. **A URL cited in the issue or in the PR** — description or comments. The author's own reference wins over anything the review finds on its own, because it is the contract the change was written against.
...
- Always run:
    ...
    - @skills/security-review/SKILL.md — **skipped when `SECURITY_OWNER=athena` is set by the caller.** When `athena` runs as a separate, parallel security pass, this inline pass repeats the same full security review over the same diff and lands the same findings twice; the always-run set is strictly sequential, so the duplicate is pure added latency, not added coverage. **Absence of the flag means it runs** — coverage never drops because someone forgot to set it, and a run without `athena` is unchanged. When it is skipped, say so on the summary line (`security: owned by athena`) so the PR comment never implies a security pass that did not happen here.
