...
- **Never use a git worktree for the writing path.** Every task runs on the single shared working tree (`agents/daedalus.md` *Worktrees — writing path stays on the shared tree*). A read-only code-review worktree stays the CR agent's own opt-in and is never requested by this skill
...
- **Delegate the whole task to `daedalus`** through the Task tool, one dispatch per task, with the issue URL (or the adopted PR URL), the explicit instruction to take it through to a merged pull request, and the constraint that no worktree is used on the writing path. `daedalus` owns claim-labelling, implementation and scoped validation (`hephaestus`), the review-and-fix loop (`athena` ↔ `hephaestus`) to convergence, the post-convergence report (`hermes`), and the merge chain.
- **For an adopted PR, say so in the dispatch prompt**: state that the implementation already exists on the PR's branch and the run starts at the review-and-fix loop. `daedalus` resolves a URL into a subject and then dispatches `hephaestus` to implement (`agents/daedalus.md` step 5), so an adopted PR handed over without that instruction is implemented a second time.
