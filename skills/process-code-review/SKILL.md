...
- **No build gate here.** The full gate is not run at Finalization either — it runs once immediately before the merge (`@skills/resolve-issue/references/quality-gates.md` *Gate placement — deferred to the merge boundary*), on the exact head commit being merged, where `@skills/merge-github-pr/SKILL.md` *Pre-merge quality gate* also owns the fix-commit and re-review rules. Convergence of this loop is a **review** verdict (`criticalCount + moderateCount == 0`), never a build verdict; the two gates are separate and the merge requires both.
