...
- **Run the full quality gate now — this is the branch's single gate run.** The loop above deliberately ran no fixers and no checkers, so this is the first and only point where they execute (`@skills/resolve-issue/references/quality-gates.md` *Gate placement — deferred to the merge boundary*). Convergence of the loop is a **review** verdict; this is the **build** verdict, and the merge requires both. Run the project's full gate (`composer build`, the Phing target, or the project's equivalent) on the current head.
  - **Green on the first run → nothing more to do here.** Record the command and its result for the `cr-status` comment.
