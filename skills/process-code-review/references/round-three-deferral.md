...
- **Apply one content label** per `@rules/compound-engineering/general.md` *Label tracker issues, and keep the labels true*. Selecting it is a semantic judgment the caller makes from the repository's existing labels; the script only applies the label it is given, and no label is a permitted outcome.
