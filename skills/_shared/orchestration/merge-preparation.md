...
Extracted from `agents/daedalus.md` so the orchestrator does not carry it on every run. **Loading trigger:** the user invokes `/prepare-issue-for-merge <URL>`, `$verify-merge-readiness`, or asks in words to prepare an issue's pull request for merge without merging it. On every other run this file is not read.
