...
- Extract two token sets from the block text — before-edit and after-edit: every `### slug` / `[[slug]]` reference, every URL, every `#<number>` issue/PR reference, every commit SHA, and every concrete pointer (a `path/to/file.ext`, optionally `:line`, or a bare class/method/script name quoted in backticks).
