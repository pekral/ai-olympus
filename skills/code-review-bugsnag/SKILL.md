...
- **Repository ownership (hard gate).** Confirm the linked PR belongs to the current checkout by running `skills/_shared/assert-current-repo.sh <PR_URL>` before checking it out. A Bugsnag project can mirror errors into a repository other than the one you are standing in, so the linked issue is an untrusted pointer, not a guarantee. Exit code `4` means the PR lives elsewhere: **stop**, report the mismatch, and do not review or comment. Exit code `5` means ownership could not be proven (not a git checkout, or no github.com remote on any of them): stop and tell the caller to run from inside the target checkout. Only a zero exit permits the flow to continue — every non-zero exit is a hard stop, and the deterministic loader's "exit 2/3 → fall back to the MCP server" convention never applies to this guard: there is no fallback for an ownership verdict.
...
- Run conditionally (same triggers as `@skills/code-review-jira/SKILL.md`): refactoring diff → `@skills/refactor-entry-point-to-action/SKILL.md` with `MODE=cr`; database operations → `@skills/mysql-problem-solver/SKILL.md` (surface under `## Database Analysis`); shared state → `@skills/race-condition-review/SKILL.md`; third-party API changes → the Third-Party API & Service Analysis step from `@skills/code-review/SKILL.md`, including the ordered documentation source walk (step 2) and, when it resolves no reference, the blocking documentation request (step 7), published on the linked GitHub PR comment under `## Documentation Requests` (see Output Rules).
...
- **`## Documentation Requests` section (issue #151).** Render this section on the linked GitHub PR comment — after `## Findings`, before `## Refactoring (DRY / tech debt)` — **only** when the Third-Party API & Service Analysis step 7 in `@skills/code-review/SKILL.md` produced at least one blocking documentation request. Each entry carries the vendor / service, the version in use (or an explicit `could not determine`), the endpoints / SDK methods / webhook events being verified, and the one-sentence ask for the documentation link. The PR comment is the surface the fix's author reads, so the request lives there; the Bugsnag error comment stays non-technical and never carries it. Omit the heading entirely when no request exists.
- **`## Excluded per assignment` section (issue #17).** This wrapper's step 1 (`skills/code-review-bugsnag/scripts/load-issue.sh` plus `skills/code-review-github/scripts/load-issue.sh` for the linked PR) already loads the Bugsnag error `comments[]`, the linked-issue comments, and the linked PR's description and review comments — exactly the first-class assignment sources the Assignment-Declared Test-Only Conditions — Exclusion Gate (`@rules/code-review/general.mdc` *Assignment-Declared Test-Only Conditions — Exclusion Gate (issue #17)*) requires for detection condition 1. Render the section on the GitHub PR comment only when `@skills/code-review/SKILL.md` moved at least one finding into it; omit it entirely otherwise.
