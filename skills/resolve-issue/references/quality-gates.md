...
- **A rebase repeats the gate only when it resolved a conflict.** There is no gate before the merge boundary, so a rebase during the branch's working life has nothing to repeat. At the merge boundary the question is whether the rewrite produced content no build has seen, and the effective PR diff fingerprint answers it.
  A cleanly replayed patch keeps its fingerprint however far the base moved, so the recorded run carries forward. A resolved conflict is hand-written content: it changes the fingerprint and re-runs the gate on the new head.
  A clean rebase still pulls new upstream commits into the merged tree, so the carry-forward also requires green CI on that head — the workflow builds the merge ref, which is the only evidence the combination holds. The full predicate lives in `@rules/git/general.md` *A history rewrite re-runs the gate only when it resolved a conflict*; the same fingerprint comparison decides the code-review round (`@rules/code-review/general.md` *Incremental Review Scope — Diff Since the Last Reviewed Revision*).
...
## Classify a red gate before you fix it

A red gate is not evidence that the diff is broken. The same red appears when the machine running the gate cannot serve it — no test database, a connection pool a parallel run already exhausted, a browser server that never started. Reading every red as a code defect is how a run "fixes" a change that was already correct: it rewrites working code until the unrelated failure stops appearing, then lands that rewrite as the gate commit.

**Classify first, fix second.** Before changing one line, confirm the failure is reachable from the diff — name the changed file the failing test exercises, or the line the branch added that the checker flags. When no such line exists, the failure is not this branch's.

**These messages describe the environment, never the code:**

- `Too many connections` — the database refused a new connection; the pool is exhausted, usually by a parallel run.
- `Unknown database` — the test database does not exist on this machine.
- `Base table or view not found` / `Base table ... does not exist` — the schema was never migrated, or a parallel run dropped it mid-suite.
- `playwright-server.json` — the browser server is not running, so the end-to-end suite has nothing to drive.

The list names the common cases and is not exhaustive. Any failure whose message describes the environment rather than the code belongs in the same bucket.

**What to do with one.** Report it — the exact message and the command that produced it — then re-run the gate. A transient exhaustion clears on the second run. A missing database or a missing browser server does not: it needs the environment repaired, which is a hard stop for a human to resolve (`@skills/merge-github-pr/SKILL.md` *Pre-merge quality gate*, **A gate that cannot be run is a hard stop**). **Never change code because of an infrastructure failure, and never commit a change made to silence one.**

**The classification is never an escape hatch.** A real failing test, a real checker error, and a real coverage shortfall block exactly as before, and the merge still requires a green gate on the exact head commit. When the message is ambiguous, treat it as a code defect and investigate: the cheap outcome of a wrong guess there is one wasted investigation, and the expensive one is a broken change merged under a red somebody dismissed.
