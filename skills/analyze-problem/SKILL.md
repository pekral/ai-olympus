...
7. **Smallest safe solution** — the smallest, lowest-risk fix that addresses the root cause. When step 6 found damaged data, the solution has **two ordered halves**: fix the cause first, then repair the records it already wrote, per `@rules/compound-engineering/general.md` *Fix the cause first, then repair the data it already wrote*. Never propose the repair first or alone — a repair that runs while the cause is live is re-corrupted by the next request that takes the broken path.
