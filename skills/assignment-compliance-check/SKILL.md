...
- The returned block carries **no `Authors` line and no `Available behind` line**. Both were resolved by `@skills/pr-summary/SKILL.md`, which resolves neither now, and neither belongs here anyway: the block is embedded inside that skill's own report, which already states whose change it describes and how to reach it. The block reports gaps and nothing else.
