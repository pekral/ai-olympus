...
- Every merge runs the project's full quality gate on the exact head commit first (*Pre-merge quality gate*) — it is the branch's only gate run, and a merge never proceeds without it passing
...
The project's fixers and checkers do not run during the branch's working life — implementation and the review loop both skip them by design (`@skills/resolve-issue/references/quality-gates.md` *Gate placement — deferred to the merge boundary*). This step is therefore the **only** execution of the full gate for the whole branch, and it is where the fixes it produces are committed. It runs on every merge, independently of CI status and of the billing exception above.
...
1. **Check out the exact head commit being merged** and confirm a clean tree (`git status --porcelain --untracked-files=all` is empty). A dirty tree invalidates the run — what the gate proves would not be what gets merged.
   - **A gate run already performed on this exact head commit counts.** `@skills/process-code-review/SKILL.md` *Finalization* runs the same full gate after its review loop converges, so on a PR that went through that loop the gate has normally already passed on this commit. Accept it and skip to step 6 when **all** of the following hold: the run is recorded in the PR's `cr-status` comment with the command and its result, the head SHA has not moved since (no commit was pushed after that comment's `createdAt`), and the tree is clean. Otherwise run the gate here. This is the whole point of deferring the gate — a merge must never re-prove, on the same bytes, what a recorded run already proved.
2. **Run the project's full gate** — `composer build` (install + fixers + full `check`, including full-suite coverage), the Phing target, or the project's equivalent, discovered per `@skills/resolve-issue/references/quality-gates.md`.
3. **Green on the first run, clean tree → proceed to the merge.** Record the command and its result in the merge report.
4. **Anything reported → resolve it and commit the result as a new commit on the branch.** Never amend a commit already under review and never force-push a branch a reviewer has commented on (`@rules/git/general.md`). Use `chore(gate): apply pre-merge fixer and checker fixes` when the commit carries only tool output, or a `fix(scope):` subject when resolving it changed behaviour. Push the commit.
5. **Re-run the gate on the new head** and repeat from step 2 until it passes on the exact commit being merged. A merge never proceeds on a head commit whose own gate run did not pass.
6. **Re-derive the code-review gate against the new head.** The fix commit moved the head SHA, so the converged review's `createdAt` now predates it and *Converged code review on the final diff* in step 2 would read it as stale. Which outcome applies depends on what the fix commit changed:
...
7. **A gate that cannot be run is a hard stop.** If the full gate cannot execute (missing tooling, broken install, unavailable dependency), **do not merge** — report what failed. There is no path on which a merge proceeds without a green gate on the merged commit; the *merge anytime* waiver below covers only the missing **CI** signal, never this gate.

**Savings-mode cache reuse (opt-in, never a weaker check).** When the shared brief records `## Savings mode: on`, step 2 may be satisfied by a cached passing entry from the brief's `## Build gate cache`, under exactly the provenance requirements the *GitHub Actions billing exception* above states for its substitute build: the entry's recorded hash must equal the tree hash of this exact head commit, and it must carry this-run provenance. A miss always runs the gate here, now, on this head SHA.
