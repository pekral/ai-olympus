...
1. **Check out the exact head commit being merged** and confirm a clean tree (`git status --porcelain --untracked-files=all` is empty). A dirty tree invalidates the run — what the gate proves would not be what gets merged.
   - **A gate run already performed on this exact head commit counts.** `@skills/process-code-review/SKILL.md` *Finalization* runs the same full gate after its review loop converges, so on a PR that went through that loop the gate has normally already passed on this commit. Accept it and skip to the merge step when **all** of the following hold; otherwise run the gate here. This is the whole point of deferring the gate — a merge must never re-prove, on the same bytes, what a recorded run already proved.
     ...
     - **The record covers the bytes being merged.** The `Quality gate:` line must carry the head SHA the gate ran on, and that SHA must equal `git rev-parse HEAD` of the commit being merged.
       A branch rebased since the run is the one case where a different SHA still counts, because a catch-up rebase is exactly what `mergeStateStatus == "BEHIND"` forces before a merge (`@rules/git/general.md` *A history rewrite re-runs the gate only when it resolved a conflict*). It counts only when **all three** hold:
       1. the `Quality gate:` SHA equals the comment's own `Reviewed revision:`, so the comment's `Reviewed diff fingerprint:` is the fingerprint of the gated bytes;
       2. that fingerprint equals one recomputed on the current head — a conflict resolved during the rebase changes it;
       3. every entry in `statusCheckRollup[]` is green on that head, which is what proves the combined tree builds — a CI that carried no signal (the billing exception below) leaves it unproven.
       Either of the last two failing runs the gate here.
       Compare the SHA and the fingerprint themselves — **never a timestamp proxy.** "No commit was pushed since the comment" is not a safe test: GitHub exposes no push time, and a rebase preserves `authoredDate`, so a stale run would pass a time-based check while the bytes being merged were never gated.
...
5. **Anything reported → classify it, then resolve it and commit the result as a new commit on the branch.**
   - **Classify it first** per `@skills/resolve-issue/references/quality-gates.md` *Classify a red gate before you fix it*. A failure describing the environment — an exhausted connection pool, a missing test database, an unmigrated schema, a browser server that never started — is reported and the gate re-run, never fixed in code, and a change made to silence one never enters the merged diff. Never amend a commit already under review and never force-push a branch a reviewer has commented on (`@rules/git/general.md`). Use `chore(gate): apply pre-merge fixer and checker fixes` when the commit carries only tool output, or a `fix(scope):` subject when resolving it changed behaviour. Push the commit.
