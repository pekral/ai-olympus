...
Every CR run **updates the one `cr-comment` it owns** on the PR rather than adding another (`@skills/code-review/SKILL.md` *Cross-run history*), so the newest trusted comment is the only one. Treat the newest trusted comment as current when either its `Reviewed revision:` equals `headRefOid`, or its `Reviewed diff fingerprint:` equals a freshly recomputed fingerprint of the current effective PR diff using the canonical command in `@rules/code-review/general.md`.
