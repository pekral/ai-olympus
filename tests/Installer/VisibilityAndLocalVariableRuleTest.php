...
test('the PHP standards require private visibility for a method only its own class calls', function (): void {
    ...
    expect($standards)->toContain('**A string or array callable runs in the scope of the code that receives it.**');
    expect($standards)->toContain('replace the reference with first-class callable syntax (`$this->name(...)`), which a private method supports.');
...
test('the PHP standards forbid widening visibility for a test and name the alternatives', function (): void {
    ...
    expect($standards)->toContain('and never use it to reach a private member (see *Never widen visibility for a test* below).');
...
test('the code review walks the three rules at their declared severities with gating', function (): void {
    ...
    expect($crRule)->toContain('for such a method the finding and its fix include replacing the reference with first-class callable syntax');
