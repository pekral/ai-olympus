...
- **Result:** inline drafts — tweet, thread, release notes, marketing summary — or a link to the published comment when `Published` / `Reporting done`. In reporting mode this is the `Summary of changes` + `How to test` report, and it names the `hephaestus` handoff for the current head SHA the test steps were built from.
