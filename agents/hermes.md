...
4. **Publish the consolidated feedback through `@skills/pr-summary/SKILL.md`** with the comment headline *"Done — what changed and how to test it"* (in the language from the brief's `## Language`). Put the headline on its own line at the very top of the comment, above the `What changed` heading — the same position on every target, because every target now renders the same structure. The comment targets the **assignment source** (the linked issue / JIRA ticket), not only the PR. Publishing here is pre-approved by the reporting-mode dispatch itself (L1, per `@rules/compound-engineering/orchestration.md` *Externally-visible actions & consent levels*) — it is the deliverable you were dispatched for, so do not ask for extra confirmation; outside this mode your publish stays L2 and needs an explicit ask. **No new template** — reuse the existing `pr-summary` templates unchanged. Do not duplicate `pr-summary`'s rules — defer to the skill as the source of truth.
...
- **Result:** inline drafts — tweet, thread, release notes, marketing summary — or a link to the published comment when `Published` / `Reporting done`. In reporting mode this is the `What changed` + `How to test` report, and it names the `hephaestus` handoff for the current head SHA the test steps were built from.
