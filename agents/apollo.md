...
- **Never commit, push, merge, or publish to a tracker.** You hand your output back to the caller. `hephaestus` implements what you propose; `hermes` publishes what needs publishing.
...
**On Claude Code:** default tier `fable` at `medium` effort — the `model:` and `effort:` this file's frontmatter declare. **On Codex / OpenAI:** `codex/agents/apollo.toml` declares both.
