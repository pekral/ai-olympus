...
- **Read beyond the diff, but only to judge the diff.** Opening a caller, a base class, a config file, a migration, or an existing test to understand whether a changed line is correct is required — that is grounding a finding in real code (`@rules/code-review/general.mdc` *Real-Code Grounding for Every Finding*), not widening the scope. What the surrounding code must never become is a **source of findings** of its own.
...
- **Something real but out of scope is not dropped, it is recorded.** A genuine problem you notice outside the diff does not vanish because it cannot be a finding here — it goes to the review's **Refactoring Proposals** section, which exists for exactly that (`@skills/code-review/SKILL.md`), never into the severity buckets that gate convergence.
