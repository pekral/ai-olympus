...
2. **Heuristic — scoped vs. full build:**
   ...
   - **Savings-mode build-gate cache (opt-in).** Before running a **full** `composer build` in this step, when the shared brief records `## Savings mode: on` (`@rules/compound-engineering/general.mdc` *Savings mode*), check the brief's `## Build gate cache` for a passing entry keyed to the current tree's exact content hash and cite it instead of re-running when it matches; on a miss, run the full build as usual and append the result to the brief so a later step (or the eventual merge gate) can reuse it. This never applies to the mandatory full run on the exact final head SHA immediately before merge.
