...
**Security agenda:** security review is owned by `athena`, not argos. When the caller dispatches `athena` alongside this run, pass `SECURITY_OWNER=athena` into the wrapper so its always-run `security-review` pass is skipped — the always-run set is sequential, so running it here as well only adds latency and duplicate findings. Never set the flag when `athena` is not actually running: the default (unset) keeps the inline pass, and that is what guarantees coverage never silently drops. `athena` runs as a separate, dedicated security CR pass and returns / publishes its own security findings; argos does **not** duplicate that coverage. When `athena` is not registered, the CR skills (`code-review-github` etc.) already invoke `@skills/security-review/SKILL.md` inline as part of their pipeline — argos relies on whatever that inline pass produces and does not re-run it.
