...
`splinter` runs the full delivery route with the redesign stage. `michelangelo` writes the proposal and one preview per state, `donatello` implements it with the existing design system and opens the pull request, `leonardo` reviews it to convergence, and `raphael` verifies the page in its own interactive browser on desktop and mobile viewports. The workflow keeps the main layout shell and the business logic unchanged, and it never merges.
