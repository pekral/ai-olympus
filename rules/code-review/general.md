...
**What is still reported (unchanged).** The header block, `## Findings` → Critical and Moderate sub-headings, `## Documentation Requests`, `## Database Analysis`, `## Architecture`, `## Coverage`, `## Excluded per assignment`, `## Functional Review`, and the `Summary` line. `## Excluded per assignment` stays because it is an **audit record**, not a finding — the Exclusion Gate above requires that every finding it relocates is recorded and never silently dropped, and this scope rule does not override that.
