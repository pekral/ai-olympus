...
- Never change JIRA issue status, with three exceptions:
  1. A transition to the project's In Progress status, performed only via `skills/code-review-jira/scripts/transition-to-in-progress.sh` — either an early transition at the start of work, or the owner's return from a review-phase status (Code Review, Ready to Merge) when work on the code resumes.
  ...
  Reverting exception (3) — a later commit re-opens the review, so the issue moves back to the review column — needs no fourth helper: that move is exception (2)'s own transition, run again. Reverting exception (1) needs no fourth helper either: the owner's return from review is exception (1)'s own transition, run again. Each project may name these columns differently, so the target is resolved and validated by each helper, never hardcoded.
- The In Progress helper accepts the Czech status name `Rozpracováno` as a built-in safe progress target.
Before changing an issue that is already in progress, in review, or in Ready to Merge (matched the same synonym-aware way `transition-to-code-review.sh` and `transition-to-ready-to-merge.sh` match their own targets), it verifies that `assignee = currentUser()`; otherwise it aborts instead of stealing another run's claim. The same check lets the owner return an issue from either column to In Progress when work on the code resumes.
A finished issue (Done, Closed, Resolved, Cancelled, Canceled, Merged) is never returned. For a new claim it assigns the issue to the account currently authenticated in `acli` by running `acli jira workitem assign --key <KEY> --assignee "@me" --yes`, then verifies the same JQL predicate. A failed or unverified assignment blocks implementation even when the status transition succeeded.
