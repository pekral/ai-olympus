...
- The In Progress helper accepts the Czech status name `Rozpracováno` as a built-in safe progress target. Before changing an issue that is already in progress or in review, it verifies that `assignee = currentUser()`; otherwise it aborts instead of stealing another run's claim. The same check lets the owner return an issue from review to In Progress when work on the code resumes. A finished issue (Done, Closed, Resolved, Cancelled) is never returned. For a new claim it assigns the issue to the account currently authenticated in `acli` by running `acli jira workitem assign --key <KEY> --assignee "@me" --yes`, then verifies the same JQL predicate. A failed or unverified assignment blocks implementation even when the status transition succeeded.
