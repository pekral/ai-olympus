...
- The In Progress helper accepts the Czech status name `Rozpracováno` as a built-in safe progress target. It also assigns the issue to the account currently authenticated in `acli` by running `acli jira workitem assign --key <KEY> --assignee "@me" --yes`. It verifies the result with `key = <KEY> AND assignee = currentUser()`. A failed or unverified assignment blocks implementation even when the status transition succeeded.
