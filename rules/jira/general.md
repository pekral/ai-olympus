...
- **A helper that fails is never a licence to improvise.** When `upsert-comment.sh` exits 2 or 3, the only sanctioned fallback is the JIRA MCP server with an **ADF** payload. Never retry with a raw `acli` write, and never hand the Wiki Markup source to any other command — that is how a ticket ends up carrying `h2.`, `{{code}}`, and `[label|url]` as literal text.
