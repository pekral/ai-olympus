...
- Each **specialist agent** (`hephaestus`, `athena`, `hermes`, `argus`, `apollo`) reads only the entries where `Role:` matches its own role **or** `Role: shared` — narrowing the context to the lessons it can directly act on.
...
- **The filing bar — exactly two things are filed, and nothing else is.** An agent creates a follow-up tracker issue for one of these two, and for nothing else:
  ...
  2. **A critical security issue** — still subject to *Never disclose an unfixed vulnerability on a public tracker* in `agents/athena.md`, which governs *where* it is filed, never *whether*.
