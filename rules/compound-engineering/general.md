...
- **What must be filed.** Every item the run itself records as deferred future work: entries in the PR `## TODO` checklist, entries under a `## Deferred to sub-issues` heading in the published CR comment, and pre-existing issues deferred as non-trivial. A point **rejected with a stated reason** (decided against, not postponed) is not filed — rejection is a decision, deferral is a promise.
