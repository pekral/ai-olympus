...
| `donatello` | Write the claim label (`Resolve_by_AI:in-progress`) on the source issue, and release it on a `Blocked` stop | L1 | `@rules/compound-engineering/general.md` *Claim a tracker issue before working on it* — mechanics in `@skills/resolve-issue/SKILL.md` |
| `donatello` | Write the review-waiting phase signal on the source issue once the PR is open (GitHub `ready for review` label, creating it when the repository lacks it; JIRA Code Review transition) | L1 | `@rules/compound-engineering/general.md` *Tracker status tracks the phase of work* — mechanics in `@skills/resolve-issue/SKILL.md` |
...
| `leonardo` | Write the ready-to-merge phase signal on the source issue when the review converges (GitHub `ready to merge` label, creating it when the repository lacks it; JIRA Ready to Merge transition), and withdraw it when a later commit re-opens the review | L1 | `@rules/compound-engineering/general.md` *Tracker status tracks the phase of work* — mechanics in `@skills/process-code-review/SKILL.md`; the same converged-loop precondition as the Draft promotion above |
