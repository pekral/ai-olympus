...
4. **Order the pieces** — blockers before dependents, then by the priority taxonomy from *Triage mode*. Report that order; it is what the user re-runs the orchestrator against, one piece at a time.
