...
- **Never let tests perform a real DNS lookup or open a raw network socket.** Production code calls `dns_get_record()`, `gethostbynamel()`, `fsockopen()`, and similar functions only through one injectable class. `tests/TestCase.php` binds a fake of that class in `setUp()` with `$this->app->instance(...)`, so no test reaches the real resolver. The full contract is `@rules/code-testing/general.md` *External Calls*.
