...
- **Active voice, named actor.** Write *`hefaistos` opens the pull request*, never *the pull request is opened*. The passive hides who acts, and in an agent pipeline the actor is the load-bearing part.
...
> **Scope boundary — style, not language choice.** This rule owns **how** a sentence is written. `@rules/reports/general.mdc` owns **which language** a tracker-published report is written in (the assignment's language, never mixed) and the narrow English exception for technical CR findings on a GitHub PR. The two never overlap and never override each other: a report first takes its language from `@rules/reports/general.mdc`, then obeys this rule inside that language. Raise one finding per violation — a Czech report written in bloated prose is a violation of this rule only, and an English report dropped into a Czech tracker comment is a violation of `@rules/reports/general.mdc` only.
