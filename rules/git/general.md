...
- **A `FAST`-tier run has no review loop to converge, so `daedalus` promotes it.** When adaptive routing classifies the change `FAST` on its final diff, no `athena` pass is dispatched and `process-code-review` never runs, so nothing would otherwise take the pull request out of Draft. `agents/daedalus.md` *Adaptive routing* → *Closing a `FAST` run* owns that promotion, and it is gated on the same two facts the loop would have proven: the final classification is still `FAST`, and `hephaestus`'s scoped validation returned green. Every other tier keeps the rule below unchanged.
