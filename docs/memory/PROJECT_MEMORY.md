...
### package-purpose-and-runtime-surfaces — AI Olympus distributes one governed development workflow to Claude Code and Codex
- Trigger: explaining the package, changing an installation surface, or deciding whether a proposed feature belongs in its scope.
- Rule:    AI Olympus is a Composer-distributed PHP/Laravel agent toolkit, never an application runtime. Its tracked `rules/`, `skills/`, and five canonical `agents/*.md` roles implement an issue-to-reviewed-PR workflow: implementation, code/security review, acceptance testing, tracker reporting. `vendor/bin/ai-olympus install` projects those sources into Claude Code and Codex without an editor selector, and is the binary's only subcommand — the package ships no process-spawning code (the `resolve-next` scheduler was removed 2026-09-08; `cron` plus `claude -p` replaces it). Claude-only permission settings stay Claude-only, and the Claude plugin marketplace channel is not a Codex distribution mechanism.
- Example: the 2026-09-05 Codex integration added native Codex locations and adapters, preserving the Claude workflow and canonical role prompts.
...
- Rule:    Registration status is point-in-time, not permanent. Before implementing anything scoped to "every agent", compare the canonical `agents/*.md` roster with `.claude/agents/` and `.codex/agents/*.toml` (or their source equivalents in this package), plus the current installer roster tests — treat live files as source of truth over memory/issue text. A stale premise produces incomplete parity the CR loop flags as Moderate.
...
- Rule:    Verify `src/Installer.php`, `src/InstallerPath.php`, `src/InstallerClaudeSettings.php`, `src/ComposerPlugin.php`. The `--editor` flag was removed in issue #16 / PR #33; the installer now targets Claude Code and Codex automatically. (1) `Installer::install()` passes opt-in `--disable-co-author-attribution` to `applyCoAuthoredByPreference()`: writes `includeCoAuthoredBy: false` into `~/.claude/settings.json` only when requested and absent (never overwrites); defaults do not read global settings. (2) Payloads write both harness trees; `--global` adds both home skill locations. (3) `allow-plugins: true` in `composer.json` enables `ComposerPlugin`'s `post-install-cmd`/`post-update-cmd` hook, which (when `extra.ai-olympus.auto-install: true`) runs `--force` on every `composer install`/`update`: code execution without further confirmation. Describe all three surfaces.
- Example: PR #672 (issue #664): SECURITY.md omitted the then-unconditional `includeCoAuthoredBy` write (Critical) and `allow-plugins` hook (Moderate). Refs: `src/Installer.php:91`, `src/InstallerClaudeSettings.php:60-95`, `src/ComposerPlugin.php:43-69`.
- Source:  https://github.com/pekral/ai-olympus/pull/672   Added: 2026-06-22   Updated: 2026-09-09 (co-author preference made opt-in for planned 0.1)
- Role:    shared
...
### codex-agent-adapters-share-canonical-role-prompts — Codex adapters must load the canonical agent Markdown instead of duplicating it
- Trigger: adding, renaming, or changing an AI Olympus agent while maintaining Claude Code and Codex parity.
- Rule:    Keep the complete workflow and safety contract only in `agents/<name>.md`. The installer mirrors it to `.claude/agents/<name>.md` and `.codex/agent-instructions/<name>.md`; `.codex/agents/<name>.toml` stays a thin adapter loading that Markdown and translating only harness paths/tool names. Keep adapter basenames equal to the canonical roster, omit `model` so Codex inherits the caller's, and pin the one-to-one mapping in tests. Global Codex mappings belong in root `AGENTS.md`: `.claude/run` → `.codex/run`, `@skills` → `.agents/skills`, `@rules` → `.codex/rules`. Never fork the full agent prompt into TOML — the two harnesses would silently diverge on the next workflow edit.
