...
- Rule:    AI Olympus is a Composer-distributed PHP/Laravel agent toolkit, not an application runtime. Its tracked `rules/`, `skills/`, and five canonical `agents/*.md` roles implement an issue-to-reviewed-PR workflow covering implementation, code/security review, acceptance testing, and tracker reporting. `vendor/bin/ai-olympus install` projects those same sources into Claude Code and Codex without an editor selector; `resolve-next` can start the oldest eligible issue non-interactively through Claude Code by default, while `resolve-next --codex` invokes `codex exec`. Claude-only permission settings remain Claude-only, and the separate Claude plugin marketplace channel is not a Codex distribution mechanism.
- Example: the 2026-09-05 Codex integration added native Codex locations and adapters while preserving the existing Claude workflow and canonical role prompts.
...
- Rule:    Source of truth is `src/Installer.php`, `src/InstallerPath.php`, `src/InstallerClaudeSettings.php`, `src/ComposerPlugin.php` — not the CLI flag list. The `--editor` flag was removed in issue #16 / PR #33; the installer now targets Claude Code and Codex automatically. Name explicitly: (1) `Installer::install()` unconditionally calls `applyCoAuthoredByPreference()`, writing `includeCoAuthoredBy: false` into `~/.claude/settings.json` when absent (never overwrites); (2) its sync payloads write both harness trees, while `--global` adds both home skill locations; (3) `allow-plugins: true` in `composer.json` unlocks `ComposerPlugin`'s `post-install-cmd`/`post-update-cmd` hook, which (when `extra.ai-olympus.auto-install: true`) runs the installer with `--force` on every `composer install`/`update` — an ungated code-execution surface. Omitting any of these lets CR understate home writes, Codex project writes, or the auto-install hook.
...
- Rule:    Keep the complete workflow and safety contract only in `agents/<name>.md`. The installer mirrors that file to `.claude/agents/<name>.md` and `.codex/agent-instructions/<name>.md`; `.codex/agents/<name>.toml` stays a thin project-scoped adapter that loads the mirrored Markdown and translates only harness paths/tool names. Keep adapter basenames exactly equal to the canonical roster, omit `model` so Codex inherits the caller's model, and pin the one-to-one mapping in tests. Global Codex mappings belong in root `AGENTS.md`: `.claude/run` → `.codex/run`, `@skills` → `.agents/skills`, and `@rules` → `.codex/rules`. Never fork the full agent prompt into TOML, because the two harnesses would silently diverge on the next workflow edit.
- Example: the local 2026-09-05 Codex-support implementation added five TOML adapters while continuing to source all role logic from the existing five `agents/*.md` files.
- Source:  local user request   Added: 2026-09-05
- Role:    shared
