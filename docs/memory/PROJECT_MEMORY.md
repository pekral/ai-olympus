...
- Trigger: resuming a daidalos run from a shared brief written by a previous, interrupted instance, and the write-lock (`.claude/run/.daidalos-write.lock`) is held — especially when the brief's own "concurrency note" names a *different* holder than the one actually found live.
