...
user → daidalos                                         (top-level; resolves source, then dispatches via Task tool)
       ...
       Task ▶ talos   (= resolve-issue)
         │        └─ pre-PR self-check: code-review + security-review (single pass, not the authoritative review) → opens PR
         ▼
