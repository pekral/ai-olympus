...
**The trade-off you are accepting — a check, not a sandbox.** It is a question the harness asks a program you installed, so everything that stops that question being asked also stops the answer: an untrusted workspace, a session that started before you installed it, a missing or slow binary, a launch mode that runs no hooks, and — since every agent has `Bash` — an agent that deletes the entry. All of those fail **open**: the command runs. It also sees a command string, never the process tree behind it.
