...
- **Registration dependency:** dispatchable only after the installer copies `agents/athena.md` to `.claude/agents/`. Until then, security runs inline in `code-review-github → security-review` (the continuity fallback), and the pre-implementation analysis mode is skipped — `talos`'s pre-PR self-check still runs `security-review` over its own diff.
...
| `athena` | Athena, goddess of wisdom and strategic defence | wisdom + strategic vigilance → security analyst (pre-implementation, on demand) and dedicated security CR sentinel (parallel to argos) |
...
**How the language survives delegation.** When `daidalos` orchestrates, the assignment's natural language is not re-guessed at each hop — `daidalos` records it once in the shared brief's `## Language` field, writes every `Task` dispatch prompt in that language, and each specialist takes the brief's `## Language` field as the authoritative source for its reply. So a Czech request produces Czech output through the whole `talos → apollon → argos` chain, not just in `daidalos`'s own final report.
...
1. **A top-level orchestrator dispatches specialists through the Task tool.** `daidalos` runs as the top-level agent the user talks to, and spends its single nesting level dispatching `talos` / `apollon` / `argos` / `athena` directly. Each specialist then orchestrates its own skills inline — `talos` runs `resolve-issue`, `argos` runs `code-review-github`, and so on.
...
user → daidalos                                         (top-level; resolves source, then dispatches via Task tool)
       ...
       Task ▶ athena  (= security-review + laravel-security + security-bounty-hunter + security-threat-analysis — parallel security CR; dispatched by daidalos, not by argos)
         │        └─ argos: convergence loop (code-review-github + fixes, maxIterations 3); reads athena's Security CR done from brief → consolidates → 0 Critical/Moderate
...
| `argos` and `athena` independently re-derive the same diff / acceptance criteria / invariants | Shared context pack + disjoint reviewer checklists | Deriving the diff, the assignment, and the acceptance criteria from the tracker is pure input-token cost that does not vary by reviewer — computing it once and handing both reviewers the same pack halves that specific cost. Splitting **overlapping** invariants (never either reviewer's own exclusive lens) between the two removes the specific failure mode observed in production — two Moderate findings on one PR being the same defect reported twice — without reducing what either reviewer independently owns. |
