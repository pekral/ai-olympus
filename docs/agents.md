...
- **Safety:** read-only orchestrator — never analyses, implements, or reviews itself; it delegates each step by dispatching the matching specialist agent, the iteration loop is skill-driven (state lives in the skill the specialist owns), and it must be the top-level agent (not a nested subagent) per the one-level nesting rule below — that single level is what it spends to dispatch `hephaestus` / `athena`.

### <img src="../assets/agents/zeus.svg" alt="zeus avatar" width="48" align="left"> `zeus` — backlog owner / project manager

The king of the gods, who assigns each domain its owner and settles what is taken up in which order, named after **Zeus**. It owns the **backlog tier**: what the team works on next, in what order, and at what size. It sits **beside** `daedalus`, never above it — `daedalus` carries one task from source to merged pull request, `zeus` decides which task that is. Two modes. **Triage** runs `github-issue-triage` over the open backlog, seeds the priority / type taxonomy, assigns a priority to every open issue, and reports the resulting queue with the one-line reason each item holds its position. **Decomposition** takes one subject too broad for a single pull request and turns it into independently deliverable tracker issues via `create-issues-from-text`, each carrying the `## Dependencies` section `daedalus` step 1 reads to plan its resolve order. Read-only with respect to code: it never implements, reviews, merges, or publishes a report.

- **Trigger:** a backlog decision — *"triage the open issues"*, *"what should we work on next"* — or a `daedalus` dispatch when its step 1 classifies a subject as *Too broad for one PR*.
- **Orchestrates:** `github-issue-triage` (triage mode), `create-issues-from-text` / `create-issue` (decomposition mode).
- **Boundary:** the backlog tier only. It does **not** analyse, diagnose, or design — that is not a backlog decision, and the roster still carries no general (non-security) analysis agent (`@skills/analyze-problem` in the top-level session; `athena` for a security-focused analysis). Its tracker writes are **work items** (issues, labels), never **reports** on work done — reporting stays `hermes`'s alone.
- **Safety:** read-only with respect to code; merging is never a side effect of a backlog run — it stays a separate, explicitly requested step through `@skills/merge-github-pr/SKILL.md`.


### <img src="../assets/agents/argus.svg" alt="argus avatar" width="48" align="left"> `argus` — acceptance tester / QA

The hundred-eyed watchman who never closed every eye at once, named after **Argus Panoptes**. It is the only agent that **runs the application**: it starts a local instance and tests it the way a real tester would — the **API through an HTTP client that actually crosses the network** (recording the exact request and response), the **UI through a real browser** (navigating, filling, clicking, observing what renders) — and returns a per-criterion `Met` / `Not met` / `Partial` / `Blocked` verdict with the steps it performed. Neither channel substitutes for the other: a UI criterion that cannot be driven in a browser is `Blocked`, never satisfied by calling the endpoint behind the page. Read-only with respect to code — it never edits a file, authors a test, commits, merges, or publishes.

- **Trigger:** dispatched by `daedalus` after the post-convergence scoped pass, **only when the change alters behaviour a user can observe**. A pure refactor, dependency bump, docs change, test-only change, or CI change is skipped with the reason named in the route.
...
| `zeus` | Zeus, king of the gods, who assigns each domain its owner and settles what is taken up first | decides what is worked on and in what order → owns the backlog that feeds `daedalus` |
...
| `daedalus` | `Task, Read, Glob, Grep, Bash` | `Write, Edit` | resolving the source, maintaining the brief/ledgers under `.claude/run/*` | no `Write`/`Edit` tool at all | never writing any tracked file through Bash redirection |
