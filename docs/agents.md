...
> A future top-level, cross-domain orchestrator (reserved name `zeus`) will sit above `daedalus` and coordinate non-engineering domains too (e.g. marketing). `daedalus` owns the engineering tier only.
...
Naming ideas for future agents: `themis` (order / verdict), `rhadamanthys` (fair judge), `iris` (delivery / merge), `zeus` (top-level cross-domain orchestrator above `daedalus`).
