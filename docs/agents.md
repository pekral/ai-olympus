...
**On a project that opted into `--enforce-agent-bash-boundary`, the stale file also loses its own Bash rules.** `src/AgentBashBoundaryPolicy.php` is keyed by agent name, and a parity test binds those keys to `agents/*.md` exactly — so a policy entry cannot outlive the file it constrains, and no `talos` alias may be kept. `rulesFor('talos')` therefore returns `[]`, which drops the two rules that entry carried: the deny on `git push --force*` and the ask on `gh pr merge` outside `@skills/merge-github-pr/SKILL.md`. The **global** half is unaffected — the network denies still apply to an unknown agent, which is what *an unknown or absent `agent_type` falls back to the global rules and is never treated as trusted* pins. Pruning restores the per-agent half; this is the same exposure a retirement carries, stated here because a rename looks cosmetic and this consequence is not.
...
**Symptom:** the shortcut above is session-wide, so it cannot express "`athena` may not commit" without also stopping `hefaistos` from committing. You want each agent's own `## Bash boundary` block to be the thing that decides.
