# Plan — the sandbox blocks file writes for write-agents (donatello)

## Goal
When a dispatched write-agent (`donatello`) is blocked by the harness sandbox / permission mode while writing files, the run **stops and reports a clear blocker with its remediation**, instead of the main thread quietly finishing the implementation outside the delegated, reviewed pipeline. The documentation states the environment prerequisite (allow subagents to Edit/Write) and how to satisfy it.

## Architecture
The cause sits in the Claude Code harness (sandbox / permission mode for non-interactive subagents), **not** in the agent definitions — `donatello` declares `Write` / `Edit` in `tools`. The repository therefore cannot "grant" the write, but it owns two things that do resolve this case:
...
- **`agents/donatello.md` + `agents/splinter.md`** — the handoff contract: on a refused write `donatello` returns `Blocked: sandbox denied file write` with its remediation, and `splinter` escalates it to the user.
...
2. Extend the handoff contract in `agents/donatello.md` with the terminal state `Blocked: sandbox denied file write` (plus remediation), and `agents/splinter.md` with its escalation.
...
## Sources
- `agents/donatello.md` (`tools: Read, Write, Edit, Glob, Grep, Bash`) — the write is permitted at the agent level.
- `agents/splinter.md` — delegation model, one-level nesting, handoff contract.
